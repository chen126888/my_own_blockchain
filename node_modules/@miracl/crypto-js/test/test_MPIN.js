...
import VECTOR_BN254 from '../testVectors/mpin/MPIN_BN254.js';
import VECTOR_BN254CX from '../testVectors/mpin/MPIN_BN254CX.js';
import VECTOR_BLS383 from '../testVectors/mpin/MPIN_BLS383.js';

var vectors = {
    BN254: VECTOR_BN254,
    BN254CX: VECTOR_BN254CX,
    BLS383: VECTOR_BLS383
};

import VECTOR_ONE_PASS_BN254 from '../testVectors/mpin/MPIN_ONE_PASS_BN254.js';
import VECTOR_ONE_PASS_BN254CX from '../testVectors/mpin/MPIN_ONE_PASS_BN254CX.js';
import VECTOR_ONE_PASS_BLS383 from '../testVectors/mpin/MPIN_ONE_PASS_BLS383.js';

var vectors_one_pass = {
    BN254: VECTOR_ONE_PASS_BN254,
    BN254CX: VECTOR_ONE_PASS_BN254CX,
    BLS383: VECTOR_ONE_PASS_BLS383
};
...
pf_curves.forEach(function(curve) {
    ...
    describe('TEST MPIN ' + curve, function() {
        ...
        it('test MPin (Full) One Pass', function(done) {
            ...
            var rtn = MPIN.CLIENT(sha, date, CLIENT_ID, rng, X, pin, TOKEN, SEC, pxID, pxCID, pPERMIT, timeValue, Y);
            ...
            var H = MPIN.HASH_ALL(sha, prHID, pxID, pxCID, SEC, Y, Z, T);
        ...
        it('test MPin (Full) Two Pass', function(done) {
            ...
            var rtn = MPIN.CLIENT_1(sha, date, CLIENT_ID, rng, X, pin, TOKEN, SEC, pxID, pxCID, pPERMIT);
            expect(rtn).to.be.equal(0);
        ...
        it('test MPin Time Permits', function(done) {
      	...
      	it('test MPin bad PIN', function(done) {
            ...
            var rtn = MPIN.CLIENT(sha, date, CLIENT_ID, rng, X, pin2, TOKEN, SEC, pxID, pxCID, pPERMIT, timeValue, Y);
            expect(rtn).to.be.equal(0);
        ...
        if (tv_curves.indexOf(curve) != -1) {
            ...
            it('test MPin One Pass ' + curve + ' with Test Vectors', function(done) {
                ...
                vectors_one_pass[curve].forEach(function(vector) {
