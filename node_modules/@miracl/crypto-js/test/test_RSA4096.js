...
import VECTORS_RSA4096 from '../testVectors/rsa/RSA4096.js';
...
describe('TEST RSA RSA4096', function() {
    ...
    it('test RSA Enctyption/Decryption', function(done) {
        ...
        for (var vector in VECTORS_RSA4096) {

            ctx.FF.fromBytes(priv.p, Buffer.from(VECTORS_RSA4096[vector].PrivP, "hex"));
            ctx.FF.fromBytes(priv.q, Buffer.from(VECTORS_RSA4096[vector].PrivQ, "hex"));
            ctx.FF.fromBytes(priv.dp, Buffer.from(VECTORS_RSA4096[vector].PrivDP, "hex"));
            ctx.FF.fromBytes(priv.dq, Buffer.from(VECTORS_RSA4096[vector].PrivDQ, "hex"));
            ctx.FF.fromBytes(priv.c, Buffer.from(VECTORS_RSA4096[vector].PrivC, "hex"));
            ctx.FF.fromBytes(pub.n, Buffer.from(VECTORS_RSA4096[vector].PubN, "hex"));
            pub.e = VECTORS_RSA4096[vector].PubE;
